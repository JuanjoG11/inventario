...
function supabaseRequest(path, body = null) {
...
async function sync() {
    console.log("🚀 Iniciando CARGA MODELO COMPARTIDO (STOCK 3)...");
    ...
    try {
        ...
        console.log("📊 Preparando inventario COMPARTIDO (location_id: 0)...");
        const inventoryRows = [];
        for (const product of products) {
            ...
            for (const size of parsedSizes) {
                if (!size) continue;
                // SINGLE RECORD per size/product for global pool
                inventoryRows.push({
                    product_id: product.id,
                    location_id: 0, // SHARED PER ALL
                    size: size.toString(),
                    stock: 3
                });
            }
        ...
        console.log(`📤 Subiendo ${inventoryRows.length} registros de inventario global...`);
        ...
        console.log("✅ ¡MODELO COMPARTIDO LISTO!");
    } catch (err) {
